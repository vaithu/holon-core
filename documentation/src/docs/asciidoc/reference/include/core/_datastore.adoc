...
To order the expression resolvers with the same expression types, the `jakarta.annotation.Priority` annotation can be used on the `ExpressionResolver` implementation class to assign a priority order, where lower values corresponds to higher priority.
