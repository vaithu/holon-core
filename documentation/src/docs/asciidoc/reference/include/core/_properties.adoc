...
The registry supports a *priority* indication for a `PropertyValuePresenter`, which can be expressed by using standard `jakarta.annotation.Priority` annotation on presenter implementation class, where lower values corresponds to higher priority. Priority can be used when a set of *conditions* are not clearly one more restrictive than others in the same registry, so an explicit lookup order has to be defined.
...
IMPORTANT: When a `PropertyValuePresenter` is registered using service extensions an *always true* condition is used, i.e. the presenter is available for any property. Use this method only for general purpose presenters. The `jakarta.annotation.Priority` annotation can be used on presenter's implementation class to assign a priority order to the presenter.
...
The registry supports a *priority* indication for a `PropertyRenderer`, which can be expressed using standard `jakarta.annotation.Priority` annotation on renderer implementation class, where lower values corresponds to higher priority. Priority can be used when a set of *conditions* is not clearly more restrictive than another, so an explicit lookup order has to be defined.
...
IMPORTANT: When a `PropertyRenderer` is registered using service extensions an *always true* condition is used, i.e. the renderer is available for any property. The `jakarta.annotation.Priority` annotation can be used on the renderer's implementation class to assign a priority order to the renderer.
