...
import com.holonplatform.core.beans.*;
...
public class DefaultBeanIntrospector implements BeanIntrospector {
    ...
    private void init(final ClassLoader classLoader) {
        ...
        Iterable<BeanPropertySetPostProcessor> propertySetPostProcessors = ServiceLoader.load(BeanPropertySetPostProcessor.class, classLoader);
        ...
        // sort by priority
        this.propertySetPostProcessors.sort(PROPERTY_SET_POST_PROCESSOR_PRIORITY_COMPARATOR);
        // property post processors
        ...
        Iterable<BeanPropertyPostProcessor> propertyPostProcessors = ServiceLoader.load(BeanPropertyPostProcessor.class, classLoader);
        ...
        // sort by priority
        this.propertyPostProcessors.sort(PROPERTY_POST_PROCESSOR_PRIORITY_COMPARATOR);
    }
    ...
    public void addBeanPropertySetPostProcessor(BeanPropertySetPostProcessor beanPropertySetPostProcessor) {
        ...
        // sort by priority
        propertySetPostProcessors.sort(PROPERTY_SET_POST_PROCESSOR_PRIORITY_COMPARATOR);
    }
    ...
    public void addBeanPropertyPostProcessor(BeanPropertyPostProcessor beanPropertyPostProcessor) {
        ...
        // sort by priority
        propertyPostProcessors.sort(PROPERTY_POST_PROCESSOR_PRIORITY_COMPARATOR);
    }
    ...
    private <T> void postProcessBeanPropertySet(BeanPropertySet.Builder<T, DefaultBeanPropertySet<T>> propertySet,
        ...
        for (BeanPropertySetPostProcessor propertySetPostProcessor : propertySetPostProcessors) {
            ...
            propertySetPostProcessor.processBeanPropertySet(propertySet, beanClass);
    ...
    public <T> BeanPropertySet<T> getPropertySet(Class<? extends T> beanClass) {
        ...
        synchronized (cache) {
            ...
            // check data path
            ((BeanPropertySet<?>) beanPropertySet).getDataPath().ifPresent(rootBeanPath::dataPath);
    ...
    private <T> BeanPropertySet<T> buildBeanPropertySet(Class<? extends T> beanClass, Path<?> parentPath)
        ...
        // check identifiers
        properties.stream().filter(BeanProperty::isIdentifier).forEach(beanPropertySet::addIdentifier);
     ...
     * for each valid detected property.
     *
    ...
    private static BeanProperty.Builder<?> getPropertyBuilder(PropertyDescriptor propertyDescriptor,
        ...
        if (TypeUtils.isString(propertyDescriptor.getPropertyType())) {
            return StringBeanProperty.builder(propertyDescriptor.getName()).annotations(annotations);
        }
        if (TypeUtils.isBoolean(propertyDescriptor.getPropertyType())) {
            return BooleanBeanProperty.builder(propertyDescriptor.getName(),
                    propertyDescriptor.getPropertyType().isPrimitive()).annotations(annotations);
        }
        if (TypeUtils.isNumber(propertyDescriptor.getPropertyType())) {
            return NumericBeanProperty.builder(propertyDescriptor.getName(),
                    (Class<? extends Number>) propertyDescriptor.getPropertyType()).annotations(annotations);
        }
        if (TypeUtils.isDate(propertyDescriptor.getPropertyType())
                || TypeUtils.isTemporal(propertyDescriptor.getPropertyType())) {
            return TemporalBeanProperty.builder(propertyDescriptor.getName(), propertyDescriptor.getPropertyType()).annotations(annotations);
        }
        // default
        return BeanProperty.builder(propertyDescriptor.getName(), propertyDescriptor.getPropertyType()).annotations(annotations);
    }
